...
const NodeCard = ({ node, side }: { node: MemoryNode; side: "left" | "right" }) => {
  return (
    <div className={cn(
      "z-30 flex items-center gap-4 bg-[var(--color-bg)]/90 backdrop-blur-md border border-[var(--color-border-strong)] p-4 rounded-2xl group hover:border-primary/30 transition-all duration-300 w-[240px] relative",
      ...
      <div className={cn("flex flex-col gap-0.5 min-w-0", side === "left" ? "items-start" : "items-end")}>
        <span className="text-xs font-bold text-[var(--color-text-primary)] truncate w-full">{node.name}</span>
        <span className="text-[10px] text-[var(--color-text-secondary)] leading-tight line-clamp-2 font-medium">{node.sub}</span>
      </div>
    </div>
  );
};
...
export default function AiMemorySectionOne() {
  ...
  return (
    <section ref={containerRef} className="relative min-h-screen flex flex-col items-center justify-start overflow-hidden bg-[var(--color-bg)]">
      {/* Background Infrastructure */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="h-[800px] w-[800px] rounded-full bg-primary/5 blur-[120px]" />
        </div>
      ...
      <div className="relative z-10 w-full flex flex-col items-center w-full">
        <div className="grid lg:grid-cols-12 gap-12 lg:gap-8 items-center w-full text-left mb-24">
          ...
          <div className="lg:col-span-7 flex flex-col items-start text-left">
            <h1 className="reveal-text text-3xl sm:text-4xl md:text-5xl font-light tracking-tight leading-tight text-[var(--color-text-primary)] max-w-2xl mb-6">
            ...
            <p className="reveal-text text-sm sm:text-base text-[var(--color-text-secondary)] leading-relaxed font-light mb-8 max-w-xl">
          ...
          <div className="lg:col-span-5 flex flex-col gap-6 pl-0 lg:pl-12 border-l border-[var(--color-border-strong)]/80 mt-8 lg:mt-0 text-left">
            {[
              {
              ...
              <div key={idx} className="flex gap-4 items-start gsap-reveal-item">
                <div className="w-5 h-5 rounded-full bg-primary/5 text-primary border border-primary/10 flex items-center justify-center flex-shrink-0 mt-0.5">
                  <Icon icon="solar:check-read-linear" className="text-xs" />
                </div>
                <div className="flex flex-col gap-1 text-left">
                  <span className="text-sm font-semibold text-[var(--color-text-primary)] leading-none">{item.title}</span>
                  <span className="text-xs text-[var(--color-text-secondary)] leading-relaxed font-light">{item.desc}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="relative w-full max-w-5xl mx-auto h-[400px] flex items-center justify-center">
          ...
          <svg className="absolute inset-0 w-full h-full pointer-events-none z-0" viewBox="0 0 1000 400" preserveAspectRatio="xMidYMid meet">
            ...
            {[70, 200, 330].map((y, i) => {
              ...
              return (
                <g key={`l-${i}`}>
                  <path d={d} fill="none" stroke="var(--color-border-strong)" strokeOpacity="0.3" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                  <path d={d} fill="none" stroke="var(--color-primary)" strokeOpacity="0.05" strokeWidth="6" filter="url(#reno-glow)" />
                  <motion.path
                    d={d}
                    fill="none"
          ...
          <div className="absolute left-1/2 top-55 -translate-x-1/2 -translate-y-1/2 z-20 flex flex-col items-center">
            <div className="relative group flex items-center justify-center">
              ...
              {/* 3D Sphere Core */}
              <div 
                className="relative flex h-40 w-40 items-center justify-center rounded-full z-10 group-hover:scale-105 transition-transform duration-500 border border-[var(--color-border-strong)]"
                style={{
                  background: "radial-gradient(circle at 35% 35%, var(--color-bg), var(--color-gradient-blue-from), var(--color-gradient-blue-to))"
                }}
              >
                {/* Specular Highlight */}
                <div className="absolute top-[15%] left-[15%] w-1/4 h-1/4 rounded-full bg-white/60 blur-[2px]" />
                
                <Icon icon="solar:database-bold-duotone" className="text-5xl text-primary" />
              </div>
            </div>
            
            <div className="mt-3 flex flex-col items-center">
               <span className="text-[10px] font-bold text-primary uppercase tracking-widest mb-1">RENO</span>
               <span className="text-xs font-bold text-[var(--color-text-primary)] tracking-tight leading-tight">Runtime<br/>Core</span>
            </div>
          </div>
        ...
        <div className="mt-28 w-full pt-12 border-t border-[var(--color-border-strong)]">
