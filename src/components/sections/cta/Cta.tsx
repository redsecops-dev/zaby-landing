          ...
          <div className="flex h-full flex-col md:flex-row">
            ...
            <div className="flex-1 p-8 md:p-12 relative z-10 flex flex-col justify-center">
              ...
              <div className="mt-8 flex gap-4">
                 <button onClick={()=> router.push("https://platform.zaby.io/tenant/signup")} className="group relative flex w-full cursor-pointer items-center justify-center gap-3 rounded-full bg-(--color-button-primary-bg) px-6 py-3.5 text-sm font-medium tracking-wide text-white shadow-[rgba(76,29,149,0.5)_0px_10px_30px_-10px] transition-all hover:bg-(--color-button-primary-hover) hover:shadow-[rgba(76,29,149,0.6)_0px_12px_34px_-10px] sm:w-auto">
                    Get Started
                 </button>
                 <button onClick={()=>router.push('/contact`')} className="flex w-full cursor-pointer items-center justify-center gap-3 rounded-full border border-(--color-button-secondary-border) bg-(--color-button-secondary-bg) px-8 py-3.5 font-medium text-(--color-button-secondary-text) transition-all hover:bg-[#e9d5ff] sm:w-auto">
                    Book a Demo
                 </button>
              </div>
            </div>
